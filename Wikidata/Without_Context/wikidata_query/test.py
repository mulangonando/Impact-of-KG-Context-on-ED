...
if __name__ == '__main__':
    #with open(os.path.join('/home/IAIS/cprabhu/Thesis/ned-graphs/', 'dataset/wikidata-disambig-dev.json')) as f:
    ...
    for checkpoint in checkpoints:
        ...
        for item in wrong_preds:
            df.loc[i] = [item.guid, item.label]
            i = i + 1
        df.to_csv('wrong_pred.tsv', sep='\t', index=False, header=True, columns=df.columns)"""