...
def get_json_data(json_data):
    ...
    for json_item in json_data:
        ...
        try:
            df.loc[i] = [i,wikidata_id_correct,text,get_graph_from_wikidata_id(wikidata_id_correct, item),_is_relevant]
            i = i + 1
            df.loc[i] = [i,wikidata_id_wrong,text,get_graph_from_wikidata_id(wikidata_id_wrong, item),_is_not_relevant]
            i = i + 1
        except Exception as e:
            print(str(e))
    return df
